    ...
    <template>
      ...
      >
        ...
        <el-table-column label="实际还书日期">
          <template slot-scope="scope">
            <i class="el-icon-date"></i>
            <span style="margin-left: 10px">{{ scope.row.true_time }}</span>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    check_w() {
      ...
      this.$axios
        .post("http://119.29.62.3/w_failhistory.php") //查看历史违规信息
        .then((Response) => {
    ...
    submit() {
      this.loading = true;
      this.$axios
        .post("http://119.29.62.3/w_brohistory.php") //查看历史借阅的所有图书
        .then((Response) => {
          let res = Response.data;
          for (var i = 0; i < res.length; i++) {
            if (res[i].fail_status == "1") {
    ...
    back() {
      this.loading = true;
      this.$axios
        .post("http://119.29.62.3/w_brohistory.php")
        .then((Response) => {
          let res = Response.data;
          this.tabledata.splice(0, this.tabledata.length);
          for (var i = 0; i < res.length; i++) {
    ...
    search() {
      if (!this.searchkey) {
        ...
        this.$axios
          .post("http://119.29.62.3/s_hisbro.php", {
            //搜索历史借阅
            b_name: bookname,
            ISBN: bookisbn,
            u_ID: userid,
